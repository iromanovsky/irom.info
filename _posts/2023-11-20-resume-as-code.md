...
![Rock Star Developer Meme](https://i.redd.it/vtomm7c1zmuz.jpg)
...
Use [JSON Resume Exporter](https://chrome.google.com/webstore/detail/json-resume-exporter/caobgmmcpklomkcckaenhjlokpmfbdec) extension for Chrome (also works in Edge) to export you LinkedIn data into JSON Resume format.
...
## Step 2. Check and amend you data if necessary
...
Carefully review the exported data for personal or sensitive information, ensuring date accuracy and relevance.
...
Take a note that different themes may display or hide different sections of you JSON Resume data, and display it differently. When yor are happy with the theme you selected, you can update `meta` section of your `resume.json` gist with the parameter `"theme": "Theme-Name"`

Now you can use your browser to "print" the web page into PDF and you are done.

Alternatively, your can use JSON Resume format to import data to some visual tools such as [ResuMake](https://resumake.io/) or even build your own theme using existing [Themes](https://jsonresume.org/themes/) and [NPM packages](https://www.npmjs.com/search?ranking=maintenance&q=jsonresume-theme) as guidance, while using 'resume-cli' command like below to build your PDF:
